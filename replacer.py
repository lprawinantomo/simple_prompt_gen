...
if __name__ == "__main__":
    ...
    llm_response_json = """{
        "refactored_method": "private void createUpdateOfficerAddress(SdmUserAccount user){\n    // Guard clause for null input\n    if (!user || !user.address) {\n        return\n    }\n    \n    Integer emcOfficerId\n    Integer officerId\n    boolean shouldTechnicalAdd = RoleUtils.hasOneOfTheRoles(user.authorization, RoleUtils.CLERK_ROLES)\n    boolean shouldPlannerAdd = shouldTechnicalAdd || RoleUtils.hasOneOfTheRoles(user.authorization, RoleUtils.EMPLOYEE_ROLES)\n    \n    if(shouldTechnicalAdd){\n        user.address.type = AddressType.TECHNICAL.code\n        AddressTO updatedEmcOfficerAddress = ConversionUtil.buildAddressTOFromSemAddressInfoData(user.address)\n        //The EMC officer address id is stored in website2\n        if(StringUtils.isNotBlank(user.address.webSite2)){\n            emcOfficerId = user.address.webSite2.toInteger()\n            AddressTO emcOfficerAddress = addressSearchService.getAddressById(emcOfficerId)\n            if(emcOfficerAddress){\n                updatedEmcOfficerAddress = ConversionUtil.populateAddressTO(emcOfficerAddress, user.address)\n                updatedEmcOfficerAddress.spectraPlusAddressId = emcOfficerId.toInteger()\n                addressManagementService.updateAddress(updatedEmcOfficerAddress)\n            }\n        }else{\n            updatedEmcOfficerAddress.spectraPlusAddressId = null\n            emcOfficerId = addressManagementService.createAddress(updatedEmcOfficerAddress)\n        }\n    }\n    \n    if(shouldPlannerAdd){\n        user.address.type = AddressType.PLANNER.code\n        AddressTO updatedOfficerAddress = ConversionUtil.buildAddressTOFromSemAddressInfoData(user.address)\n        if(user.address.spPlusId){\n            officerId = user.address.spPlusId\n            AddressTO officerAddress = addressSearchService.getAddressById(officerId)\n            if(officerAddress){\n                updatedOfficerAddress = ConversionUtil.populateAddressTO(officerAddress, user.address)\n                updatedOfficerAddress.spectraPlusAddressId = officerId.toInteger()\n                addressManagementService.updateAddress(updatedOfficerAddress)\n            }\n        }else{\n            updatedOfficerAddress.spectraPlusAddressId = null\n            officerId = addressManagementService.createAddress(updatedOfficerAddress)\n        }\n    }\n    \n    if(emcOfficerId){\n        user.address.webSite2 = emcOfficerId?.toString()\n    }\n    if(officerId){\n        user.address.spPlusId = officerId\n    }\n}",
        "changes_made": ["Added null guard clause at method entry"],
        "ast_anchors": {"method_signature": "createUpdateOfficerAddress(SdmUserAccount user)"}
    }"""
